...
function validateTradeDate(ymd) {
  const d = String(ymd ?? '').trim()
  if (!d) return 'Choose a trade date (UTC calendar day).'
  const ms = parseUtcDayStart(d)
  if (ms == null) {
    return 'Trade date must be valid YYYY-MM-DD (interpreted in UTC).'
  }
  if (ms > Date.now()) {
    return 'Trade date cannot be in the future (UTC).'
  }
  const prev = ms - 86400000
  if (prev < 0) {
    return 'Trade date is too early (no previous UTC day for volume).'
  }
  return null
}
...
async function fetchBacktest(params) {
  ...
  const res = await fetch(`/api/binance/spike-tpsl-backtest-v2?${q}`, { cache: 'no-store' })
...
function fmtVolDayChg(p) {
  if (p == null || !Number.isFinite(p)) return '—'
  return `${p >= 0 ? '+' : ''}${p.toFixed(2)}%`
}

export function SpikeTpSlBacktestV2() {
  ...
  const [onlyPriorDayUp, setOnlyPriorDayUp] = useState(false)
  ...
  const run = useCallback(async () => {
    ...
    setError(null)
    setData(null)
    setOnlyPriorDayUp(false)
    try {
      const dateErr = validateTradeDate(tradeDate)
      if (dateErr) {
  ...
  }, [minQuoteVolume24h, interval, thresholdPct, strategy, tradeDate])

  const resultView = useMemo(() => {
    if (!data) return null
    const f = data.filteredPositiveVolumeDay
    if (onlyPriorDayUp && f) return { kind: 'filtered', src: f }
    return { kind: 'all', src: data }
  }, [data, onlyPriorDayUp])

  const s = resultView?.src?.summary
  const tpLbl = data?.tpStatLabel ?? 'TP (2R)'
  const slLbl = data?.slStatLabel ?? 'SL (-1R)'

  return (
    <div className="vol-screener spike-tpsl-bt">
      <h1 className="vol-screener-title">2R backtest v2</h1>
      <p className="vol-screener-lead">
        Pick one <strong>UTC trade date</strong>. Symbols are ranked by the <strong>previous UTC day’s</strong>{' '}
        <strong>1d kline quote volume</strong> (not live24h ticker); those above your min volume are scanned on
        the trade day at your <strong>timeframe</strong>. Finds{' '}
        <strong>{strategy === 'shortRedSpike' ? 'red' : 'green'}</strong> candles whose <strong>body</strong> is
        at least your threshold % vs open.{' '}
        ...
        <code className="inline-code">(entry − exit) / entry</code>, as a percent — not compounded. After a run,
        you can restrict to coins whose <strong>volume day</strong> was <strong>up</strong> (1d open→close %) with
        the checkbox below — same payload, no extra request.
      </p>
      <p className="hourly-spikes-hint spike-tpsl-api-hint">
        ...
        <code className="inline-code">SPIKE_TPSL_V2_MAX_SYMBOLS</code> or{' '}
        <code className="inline-code">SPIKE_TPSL_MAX_SYMBOLS</code>), concurrency (
        <code className="inline-code">SPIKE_TPSL_V2_CONCURRENCY</code> or{' '}
        <code className="inline-code">SPIKE_TPSL_CONCURRENCY</code>, default 4 parallel symbols), or add{' '}
        <code className="inline-code">SPIKE_TPSL_V2_PAGE_DELAY_MS</code> /{' '}
        <code className="inline-code">SPIKE_TPSL_PAGE_DELAY_MS</code> for paging. Server-wide
      ...
      <div className="vol-screener-form">
        <label className="vol-screener-field">
          <span className="vol-screener-label">Min volume (USDT, prior UTC day)</span>
          <input
        ...
        <label className="vol-screener-field">
          <span className="vol-screener-label">Trade date (UTC)</span>
          <input
        ...
        <p className="hourly-spikes-hint spike-tpsl-range-hint">
          Example: trade date <strong>2 Apr</strong> uses <strong>1 Apr</strong> (UTC) daily quote volume to
          choose symbols, then runs the spike logic only on 2 Apr bars.
        </p>
      </div>
      ...
      {data && !error && resultView && (
        <>
          <label className="vol-screener-field spike-tpsl-v2-postfilter">
            <input
              type="checkbox"
              checked={onlyPriorDayUp}
              onChange={(e) => setOnlyPriorDayUp(e.target.checked)}
              disabled={!data.filteredPositiveVolumeDay}
            />
            <span>
              Only coins with <strong>positive</strong> prior-day % change (same UTC day as volume: 1d open→close).
              Results below switch instantly — no extra API call.
            </span>
          </label>

          <p className="vol-screener-meta">
            <strong>{data.interval}</strong> · trade day UTC <strong>{data.tradeDate ?? data.fromDate}</strong>
            {data.volumeFilterDate && (
              <>
                {' '}
                · volume filter <strong>{data.volumeFilterDate}</strong> (prior UTC day 1d quote vol) ·{' '}
              </>
            )}
            {resultView.kind === 'filtered' ? (
              <span>
                <strong> View:</strong> prior-day <strong>up</strong> only ·{' '}
              </span>
            ) : null}
            {fmtInt(data.candleCount)} bars (max/symbol) · body ≥ <strong>{data.thresholdPct}%</strong> · min
            prior-day vol {fmtInt(data.minQuoteVolume24h)} USDT ·{' '}
            {fmtInt(resultView.src.symbolCount ?? data.symbolCount)} symbols in this view
          ...
          {s && s.totalTrades > 0 && (
            <section className="hourly-spikes-section spike-tpsl-pertrade-section">
              ...
              <SpikeTpSlPerTradeLightChart
                key={`pertrade-${resultView.kind}`}
                perTradePricePctChron={resultView.src.perTradePricePctChron}
                tradesFallback={resultView.src.trades}
                totalTradeRows={resultView.src.totalTradeRows}
                serverSubsampled={Boolean(resultView.src.perTradePricePctSubsampled)}
              />
            </section>
          )}

          {resultView.src.equityCurve && resultView.src.equityCurve.length > 1 && (
            <section className="hourly-spikes-section spike-tpsl-equity-section">
              ...
              <SpikeTpSlEquityLightChart key={`eq-${resultView.kind}`} points={resultView.src.equityCurve} />
            </section>
          )}
            ...
            <div className="table-wrap hourly-spikes-table-scroll">
              <table className="positions-table">
                <thead>
                  <tr>
                    <th>Symbol</th>
                    <th className="cell-right">Prior-day vol</th>
                    <th className="cell-right">Prior-day Δ%</th>
                ...
                <tbody>
                  {(resultView.src.perSymbol ?? []).length === 0 ? (
                    <tr>
                      <td colSpan={9} className="hourly-spikes-empty">
                        No symbols or no trades.
                      </td>
                    </tr>
                  ) : (
                    (resultView.src.perSymbol ?? []).map((r) => (
                      <tr key={r.symbol}>
                        ...
                        <td
                          className={`cell-mono cell-right ${
                            r.volumeDayChangePct > 0 ? 'pnl-pos' : r.volumeDayChangePct < 0 ? 'pnl-neg' : ''
                          }`}
                        >
                          {fmtVolDayChg(r.volumeDayChangePct)}
                        </td>
          ...
          <section className="hourly-spikes-section">
            ...
            {resultView.src.tradesTruncated && (
              <p className="hourly-spikes-hint">
                Showing {resultView.src.trades?.length ?? 0} of {fmtInt(resultView.src.totalTradeRows)} rows.
              </p>
            )}
            <div className="table-wrap hourly-spikes-table-scroll">
              <table className="positions-table spike-tpsl-trades-table">
                ...
                <tbody>
                  {(resultView.src.trades ?? []).length === 0 ? (
                    <tr>
